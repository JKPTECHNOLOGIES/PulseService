...
export function useAssignTechnician() {
  ...
  return useMutation({
    ...
    onSuccess: (_data, vars) => {
      ...
      void qc.invalidateQueries({ queryKey: ["dispatch"] });
      toast.success(vars.isLead ? "Lead technician updated" : "Technician assigned");
    },
    onError: (err: unknown) => {
...
export function useRemoveTechnician() {
  ...
  return useMutation({
    mutationFn: ({ jobId, technicianId }: { jobId: string; technicianId: string }) =>
      api.delete<ApiResponse<null>>(`/jobs/${jobId}/technicians/${technicianId}`),
    onSuccess: (_data, vars) => {
      void qc.invalidateQueries({ queryKey: ["job", vars.jobId] });
      void qc.invalidateQueries({ queryKey: ["dispatch"] });
    ...
    onError: (err: unknown) => {
      toast.error(getErrorMessage(err, "Failed to remove technician"));
    },
  });
}
