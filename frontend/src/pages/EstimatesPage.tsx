...
import { PlusIcon, EyeIcon, PaperAirplaneIcon, ArrowPathIcon } from '@heroicons/react/24/outline';
import clsx from 'clsx';
import { useEstimates, useSendEstimate, useConvertToInvoice } from '../hooks/useEstimates';
...
const STATUS_FILTERS = ['all', 'draft', 'sent', 'approved', 'rejected'];
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <p className="text-sm text-gray-500">{pagination ? `${pagination.total} estimates` : ''}</p>
        <Button icon={<PlusIcon className="h-4 w-4" />} onClick={() => navigate('/estimates/new')}>
          New Estimate
        </Button>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        {isLoading ? (
          ...
          <EmptyState
            ...
            action={{ label: 'New Estimate', onClick: () => navigate('/estimates/new') }}
          />
        ) : (
          <>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-100 bg-gray-50">
                    <th className="text-left py-3 px-5 font-medium text-gray-500 text-xs uppercase">Estimate</th>
                    <th className="text-left py-3 px-3 font-medium text-gray-500 text-xs uppercase">Customer</th>
                    <th className="text-left py-3 px-3 font-medium text-gray-500 text-xs uppercase">Title</th>
                ...
                <tbody className="divide-y divide-gray-50">
                  {estimates.map((est) => (
                    <tr key={est.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="py-3.5 px-3 text-gray-700 truncate max-w-[180px]">{est.title}</td>
                      <td className="py-3.5 px-3 text-gray-500 text-xs">{formatDate(est.createdAt)}</td>
                      ...
                      <td className="py-3.5 px-5">
                        <div className="flex items-center justify-end gap-1">
                          ...
                          {est.status === 'draft' && (
                            <button
                              onClick={() => sendEstimate.mutate(est.id)}
                          ...
                          {est.status === 'approved' && (
                            <button
                              onClick={() => convertToInvoice.mutate(est.id)}
